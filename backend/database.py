...
def init_db() -> None:
    """Create tables and reset demo data so the app starts with an empty assignment list."""
    ...
    with Session(engine) as session:
        session.exec(delete(GradingResult))
        session.exec(delete(RubricCriterion))
        session.exec(delete(Rubric))
        session.exec(delete(Submission))
        session.exec(delete(Assignment))
        session.commit()
